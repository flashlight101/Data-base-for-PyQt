...
from admin.mainui import Ui_MainWindow
...
from admin.imagewindow import ImageWin
import regex as re
from admin.RangeSlider import QRangeSlider
import logging
from admin.userwinui import Ui_Users
from admin.logwinui import Ui_Log
import time
...
class LogWin(QtWidgets.QDialog,Ui_Log):
    def __init__(self):
    ...
    def init_ui(self):
        self.ui=Ui_Log()
        self.ui.setupUi(self)
        self.ui.deletelog.clicked.connect(self.deletelogfunc)
        self.tablename="Log"
        ...
        self.all_data=self.cursor.execute("select * from Log").fetchall()
        ...
        self.ui.tableUsers.setRowCount(len(self.all_data))
        row=0
...
class UsersWin(QtWidgets.QDialog,Ui_Users):
    ...
    def modifying_table(self,row,col):
        ...
        self.cursor.execute(insert,(self.ui.tableUsers.item(row,col).text(),row+2))
        self.cursor.commit()
        self.log("update "+str(self.tablename)+" set "+str(self.column[col])+" = "+str(self.ui.tableUsers.item(row,col).text())+" where id = "+str(row+2)+"")
    def log(self,sql):
...
class window(QtWidgets.QMainWindow):
    ...
    def init_UI(self,dynamic_row=10):
        ...
        self.dynamic_row=len(self.all_data)
        self.column=[elem[0] for elem in self.column]
        ...
        self.ui.tableWidget.setRowCount(self.dynamic_row)
        ...
        for tup in self.all_data:#Заполнение таблицы данными из БД
            ...
            for item in tup:
                if col==12:
                    ...
                    self.btn[row].clicked.connect(partial(self.open_image,row,self.username))#Открытие картинки в новом окне
                    #print(row)
        ...
        self.ui.logbutton.clicked.connect(self.openLog)
        self.ui.deletebutton.clicked.connect(self.delbutton)
    ...
    def openLog(self):
        ...
        self.log_window.setWindowTitle('Новое окно')
    ...
    def open_image(self,id,username):#Открытие картинки в новом окне
        self.ui.imagewindow = ImageWin(id+1,username)
